...
% mosek
% tomlab_cplex
% To change the solver, put the solver name of the list above in place of ibm_cplex in the code below. To see MATLAB version and solver version compatibility see https://opencobra.github.io/cobratoolbox/stable/installation.html#solver-installation
...
% For ease of access we will again define the folders used to store and read results from. If tutorial 1 and 1.5 are used, please use the same inputs used in those tutorials. If you downloaded the pre-made tutorial files set the path for the downloaded files as the resultPath variable. We will also define the metadata path again as we will need it later in this tutorial. Note that the explanation and steps are the exact same for the setup as in the other tutorials. If the directories already exist - the code will recognise that and not make a new one and will not overwrite the data inside of it.
...
% Now that we have checked that all our software is installed and available, our results directory has been set up and our metadata has been processed we can continue with solving our WBMs.
% 
% Next, we will choose the gurobi solver here for optimisation but any of the supported solvers will work.
solver = 'gurobi' 

% Set solver
[solverOK, solverInstalled] = changeCobraSolver(solver);
...
% Set paths
resultPath = 'D:\OneDrive - National University of Ireland, Galway\Projects_ANS_AnnaSheehy\NatureProtocol\Test tutorials';
mWBMPath = fullfile(resultPath,'mWBMmodels'); % Path to mWBMs
fluxPath = fullfile(resultPath,'resultFlux'); % Output path for the flux results
fluxAnalysisPath = fullfile(resultPath,'resultStatistics'); % Output path for the flux descriptions
...
metadataPath = fullfile(resultPath,'metadataFAKE_processed.csv');
...
table(categories, histcounts(groupIndices)','VariableNames',{'Diagnosis','Number of samples'})
...
results = performStatsPersephone(outputPathStatistics, processedFluxPath, wbmRelAbunPath, updatedMetadataPath,'',response, ...
...
% Run statistics pipeline
results = performStatsPersephone(outputPathStatistics, processedFluxPath, '', updatedMetadataPath,'',response, ...
