...
% Part 1.5 Model Personalisation
...
% We will also demonstate how to add, silence or remove reactions from the models here. You may want to personalise your WBMs in the following scenarios: 
% You have clinical/metadata for your subjects that you would like to incorporate into the model
% You want to investigate the effects of changing physiological parameters of the model
% You want to add or remove reactions to reflect a particular disease state (i.e. gene knockout)
% You want to add or remove reactions as you have discovered an update in literature that you need to incorporate into the models
...
paths.fba.outputPathFluxAnalysis = [paths.fba.fluxPath, filesep, 'fluxAnalysis'];
...
% Age: used in the calculation of cardiac output when option 6 or 7 are selected (default is option 1, CO is caluculated based on heart rate and stroke volume)
...
[iWBM, iWBMcontrol_female, iWBMcontrol_male, persParameters] = persWBM(...
    ...
    'resPath', paths.persWBM.personalizeWBMPath);
...
% Now you have a personalised WBM model and summary of the paramters which the function succesfully personalised. Talk where these can be found and what that tells us. 
% 
% Example 2: Metabolomic changes
% Blood, Urine and cerebral spine fluid metabolites can also be personalised if clinical measurements of a patient have been collected from one or more of the three biofluids. The function will than update the transport bounds of the metabolite in that biolfuid to different organs to more closely represent the indivual metabolite level of a patient. It is important that you denote the compartment in which you wish to adjust a metabolite. There are three metabolite pools in which you can adjust the bounds using this function:
...
% The metadata variable has to be formatted in a specfic way.  It is important that in the file with these physiological values the same column name as mentioned here is used to ensure the function is able to find the column in the file.
% Give a list of the parameters we can change and their corresponding column header. also give the units that can be used
% Please take care on how the units are formatted as that will deterime the conversion factors to the units used within to script to personalise the WBMs. The format of the column name for any metabolomic data you might use is important as well. The format is as follows:
...
% You'll notice here that no metIDs were found. That is because the in the VMH the metabolites are written as: 
...
% Example Five: Creating personalised HM-WMBs
...
createBatchMWBM(paths.mgPipe.mgpipePath, ...
...
% This might be used if you have some general clincal information about a group e.g., disease status, mean age, mean glucose levels and you want to make some general adjustements to the male and female models and use one (or two male and female) personalised WBMs to combine with multiple microbiome models.
% Path to your microbiome models
microbiomeDir = '';
% Folder where you want your combined mWBMs saved
saveDir = '';
% Path to metadata to obtain sex of each sample
metadataPath = '';
...
createBatchMWBM(microbiomeDir, ...
            resultPath, ...
            metadataPath,...
            'wbmDirectory', personalizeWBMPath,...
            'Diet', Diet,...
            'solver', solver,...
...
% Adding reactions:
% To investigate a particular capability of a model. For example, the examine the maximum production of a particular metabolite in a compartment, you can add a 'demand' or 'sink' reaction for that metabolite in the relevant compartment and then set it as the models objective (maximising or minimising depending of the intrinsic reaction direction and the direction you are interested in investigating). Alternatively you can add a reaction to add new metabolic functionality 
...
% Alternatively,  all reactions linked to a specific gene can be silenced- NOTE reactions linked to mulitple genes...still occur (CHECK DETAILS) (what does this mean)
% Common Applications
...
% Removing a reaction that is incorrect: Done in order to remove infeasibilities from the model or to make the model more accurate. NOTE: it is good practice to thouroughly test the model by silencing the reaction first before actually removing it.
...
% model - a metabolic model, in our case a WBM or HM WBM
...
% We will now assign values to the inputs. Good practice for creating reaction abbreviations/IDs for transport reactions is to take the organ abbreviation (if applicable), in this case Nkcells_, the VMH ID of the metabolite that is transported in all capitals, VITD3, and than lower case t to indicate transport and either and i or an r to indicate irreversible or reversible, repsectively. Lastly we indicate the compartment on the left side of the reaction where the compound and the the compartment on the right side of the reaction with a 2 inbetween to indicate which compartments are involved in the transport and which compartment is on which side of the reaction, in this case e2c
% General reaction abbreviation: Organ Abrreviation + VMH metabolite ID full caps + t + i/r + e2c
% Nkcells_VITD3tre2c
...
% Whenever you are in doubt for proper abbriviation structure look in the WBMs for similar reactions and follow those naming conventions. Because there is no transport for vitamin D3 from the blood to the natural killer cell extracellular space or from the natural killer cell extracellular space to the cytosol we will need to add both transport reactions in order to get vitamin D3 from the blood to the cytosol of the natural killer cells. If you find that the metabolite is metabolised in a different compartment within the cell, you need to add on an additional transport reaction that will go from the cytosol to the desired compartment. 
% Naming transport reactions is generally quite easy, but it is important to keep them descriptive. Good practive is to say what is transported, which organs is involved and which compartments.
...
% We will define our two formulas here. For transport reactions be aware that sometimes protons or other metabolites might be synport or antiport and you should consult the literature for the correct metabolites to use.
reactions = {'vitd3[bc] <=> Nkcells_vitd3[e]'
    'Nkcells_vitd3[e] <=> Nkcells_vitd3[c]'};
% Finally we have to indicate the gene-protein-reaction rules (gprs) and the subsystem associated with the reaction. Usually for generic transporters it is difficult to know which genes are responsible for the enzymes transporting the metabolites, if there are any. In this example we leave the gprs as an empty string ('') as we do not have information on the gprs. To find out which subsytems our reactions best belong to, we can look at the current existing subsystem in the model.
% Obtain all the subsystems in the model
subystemsModel = modelHM.subSystems;

% Convert the datatypes and get all unique values
uniqueSubsystemsModel = unique(cellfun(@cell2mat,x,'UniformOutput',false))
% We can see here a long list of available subystems. The transport from the blood compartments is usually set as Transport, biofluid. Transport from the [e] to [c] is usually set to Transport, extracellular. We do not enter the coupling factors as we take the default.
% Set the gprs
gprs = {''
    ''};
...
printFluxBounds(modelHMAddTransport, rxnAbbrs)
...
reactions = {'Nkcells_o2[c] + Nkcells_h[c] + Nkcells_nadph[c] + Nkcells_vitd3[c] -> Nkcells_h2o[c] + Nkcells_25hvitd3[c] + Nkcells_nadp[c]'};
% We do not know the gprs here so again we leave them empty. In case you do know the gprs you should use the Entrez Gene ID as used in the VMH to indicate genes. In the subystems in example 2 we can see that there is a subsystem called Vitamin D metabolism and is what we will use here.
% Set the gprs
gprs = {''};
...
printFluxBounds(modelHMVitD, rxnAbbrs)
...
printFluxBounds(modelHMExcretion, rxnID)
...
% model - A metabolic model, in our case a HM WBM or WBM
...
% value - A numeric value that say to which value the bound has to be changed to. Can be one value for all reactions, or one value for each reaction to be changed. In our case the value 0.
...
% The original model
printFluxBounds(modelHM, {'Liver_RE1303C'})

% The silenced model
printFluxBounds(modelHMSilenced, {'Liver_RE1303C'})
% As we can see, the reaction has been succesfully silenced. To understand how to use the altered model for optimisation purposes please look at tutorial 2.
...
% Now we set set the constraints of all reactions we found to 0 for both upper and lower bounds in the HM WBM
% The the value of the bounds
value = 0;
...
% The original model
printFluxBounds(modelHM, listResults(1))

% The silenced model
printFluxBounds(modelHMSilenced, listResults(1))
% And indeed we see the reaction has been succesfully silenced.

% Example 7: removing a reaction
% In special circumstances we can remove a reaction, e.g., when it interferes with the model function, if it is proven to be incorrect or if if does not connect with the rest of metabolism. Good practice, however, is to silence to reaction and througouhly test that the removed reactions does not interfere with other results and does not cause any other problems. To remove a reaction we can use removeRxns.m which takes:
...
% And indeed we see that the reaction cannot be found anymore in the model and it has been succesfully removed.
% This concludes tutorial 1.5 Personalisation.


% Example Two: Add a sink reaction
% Now lets define our reactions and run addReactionsHH. When adding a demand reaction for ATP, or in fact any reaction to the model there are a few formatting rules that need to be followed for consistnecy in the models. 
...
% Explain inputs (esp. if input name is different to function input name) This needs to be done still!
