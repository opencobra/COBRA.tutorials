%% Persephone Tutorial 5: FBA and statistics on WBMs
%% Solving Models
% Authors: Bram Nap, Tim Hensen, Anna Sheehy
% 
...
% results from. If tutorial 2, 3 and 4 were run before this, please use the same 
% inputs used in those tutorials. If you downloaded the pre-made tutorial files 
% set the path for the downloaded files as the resultPath variable. We will also 
% define the metadata path again as we will need it later in this tutorial. Note 
% that the explanation and steps are the exact same for the setup as in the other 
% tutorials. If the directories already exist - the code will recognise that and 
% not make a new one and will not overwrite the data inside of it.
%% 
...
% the output structure used in all tutorials is generated with initPersephone.m. 
...
% *mWBM_CSM5MCXD.mat* created from the test dataset in tutorial 4*.* If the tutorial 
% is followed it should be stored in the folder mWBMmodels. The model can also 
...
% files from the paper and load the model. Also, please make sure that the
% file is located in the MATLAB search path. If that is not the case, the
% pathToModel variable will be empty. You can automatically add a folder
% with all its subdirectories using addpath(genpath('PATH/TO/FOLDER')).
...
% in tutorial 4 with the EUAverageDiet. Setting the diet automatically introduces 
...
% Unconstrain the demand reaction's upper bound for analysis. Note: we
% assume that the variable 'metabolite' is a character array here. If that
% is not the case, it can be converted into a character array using char().
...
analyseGF = true;
%% 
% Lastly, if microbiome-personalised WBMs have been generated, the analyseWBMs.m 
% function also has the option to analyse corresponding germ-free versions of 
% microbiome-personalised WBMs. This functionality is triggered by setting analyseGF 
% to true. Corresponding germ-free WBMs are made by taking one male and one female 
% mWBM and setting the microbiome biomass exchange and metabolite transports to 
% 0. This ensures the microbiome cannot interact with the WBM anymore. For miWBMs, 
% a germ-free model for each miWBM. These germ-free models can serve as a baseline 
% of what the model is capable of doing without the microbiome. When we compare 
% the germfree results against the microbiome-personalised WBM results, we get 
% an indication of the microbial contribution to that specific reaction/metabolite. 
% 
...
analyseWBMs(mWBMPath, ...
        ...
        'analyseGF', analyseGF);
%% 
% After predicting and saving the WBM reaction fluxes, analyseWBMs.m calls the 
% analyseWBMsol.m function. This function performs the following steps. 1. Generate 
% a table with the FBA solution statistics. 2. Generate a table with the flux 
% predictions. 3. Produce summary statistics on the flux predictions, including 
% sample mean, standard deviation, the number of unique flux values, and the number 
% of samples for which an optimal FBA solution was obtained. 4. Remove reactions 
% from the flux table that did not meet the exclusion criterion for further analysis. 
% This criterion is a user-defined metric on i) the number or ii) the fraction 
% of unique flux values in the sampled population for a reaction, or iii) the 
% standard deviation of the flux distribution. 5. A new flux table is generated 
% with the filtered set of reactions. 6. If both microbiome-personalised and germ-free 
% WBMs were analysed, the net microbiome component of the predicted reaction fluxes 
% is calculated by subtracting the germ-free reaction flux values from the corresponding 
% predicted flux values in the microbiome-personalised WBMs. 7. Lastly, if microbiome-personalised 
% WBMs have been analysed, Spearman correlations will be performed on the microbial 
% taxonomic relative abundances and the predicted reaction fluxes.
% 
...
% 
% Before performing the statistical analysis, the predicted fluxes are z-scaled. 
% A Z-transformation was done to ensure that the statistical effect sizes, e.g., 
% odds ratios and regression coefficients, are comparable across reactions. Not 
% performing a z-transformation can result in dramatically different effect sizes, 
% as the scale of flux values can differ between reactions. If the WBMs contain 
% a gut microbiota lumen, i.e, mWBMs or miWBMs, the extracted relative abundances 
% from the WBMs are analysed in the same manner as the predicted fluxes. 
%% 
...
%% 
% The statistics can give various warning as shown when running the tutorial. 
% These warings generally occur when no good regression fit can be found. There 
% can be many reasons for a non-optimal regression fit, such as insufficient sample 
% sizes and extreme outliers. To avoid these non-optimal regression results, it 
% is important to check whether the data has sufficient sample sizes and that 
% it fits the assumptions of the performed statistical test.
% 
...
% 
% This is the end of the tutorial five and the end of all Persephone tutorials. 
%% 
% References
% 
