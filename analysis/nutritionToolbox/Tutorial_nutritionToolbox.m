...
% predict dietary changes. A database equivalent here means a food item in either 
% the USDA or FRIDA database that matches the original dietary food item both 
% on keyword and macronutrient composition.
%% MATERIALS
%% 
% * MATLAB.
...
% file. In the foodsConsumed file we will fill in the database equivalents found 
% with the foodDescription file. If a food item is not consumed in a diet, set 
% the weight consumed to 0. A worked example can be found in demoFoodConsumed.xlsx, 
...
% Database equivalents less similar to dietary food items may be chosen if they 
% have a higher measured metabolite representation in their macronutrients. By 
% choosing the database equivalents that may be less similar to the dietary food 
% item but with a higher measured metabolite representation, it increases the 
% metabolic content of the _in silico_ diets and give more overall information 
...
% and "Date", both from the USDA database, the reported macronutrient values are 
...
% and in yellow the reported macronutrients from the original diet. From the demo 
...
% ~850g, which on top of an already normal diet is quite some extra food. When 
% we look further in the dietary composition, 180g of egg white and 141g of dried 
...
% almost 20g of lipids in the diet, while we asked for the lipid conusmption to 
...
% 
% We do see that the maximum storage of glucose is blood is increases with ~3000 
% mmol/person/day, the dopamine in the cerebral spine fluuid with ~95 mmol/person/day 
