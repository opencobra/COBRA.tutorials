...
% Moreover, the tradeoff between the growth of different joined microbes 
% is computed.
% The tutorial can be adapted to any number of AGORA models and dietary
% conditions analyzed.

% We will use the AGORA resource (Magnusdottir et al., Nat Biotechnol. 2017
% Jan;35(1):81-89) in this tutorial. Please download AGORA version 1.02 
% from https://webdav-r3lab.uni.lu/public/msp/AGORA-1.02/Agora-1.02.zip
...
% combinations. NOTE: this is very time-consuming due to the large  number
% of model combinations analyzed.
...
% Set the coupling factor c, which defined how the flux through all
% reactions in a model is coupled to the flux through its biomass
% reaction. Allowed flux span through each reaction= -(c * flux(biomass))
% to +(c * flux(biomass)).
c=400;
...
% Define the number workers for parallel pool to allow parallel computing.
% Recommended if a large number of microbe models is computed. Set to zero
% if parallel computing is not available.
numWorkers=4;
...
% Diet without oxygen, Western Diet with oxygen, High fiber diet without
% oxygen, and High fiber diet with oxygen.
% Let us define the input parameters for the simulation of pairwise
% interactions.
...
% Define the corresponding constraints to implement for each diet. The
% input file needs to be a string array.
...
% NOTE: if you design your own diet, make sure that exchange reaction
% abbreviations correspond to the lumen exchanges in the joint models
% ('EX_compound[u]').
...
% growth (+), no change in growth (=) or decreased growth (-) compared with
% the single condition for each joined microbe.
% Competition  (-/-)
% Parasitism   (+/-)
% Amensalism   (=/-)
% Neutralism   (=/=)
% Commensalism (+/=)
% Mutualism    (+/+)
% This results in nine different outcomes total from the perspective of
% each joined microbe.
...
% Next, the percentage of interactions will be calculated on different
% taxon levels (genus, family, order, class, phylum) using the taxon
% information contained in AGORA_infoFile.xlsx. Here, the interactions will
% be considered from the perspective of each joined microbe resulting in
% nine possible interactions total.

% Calculate the percentage of interactions predicted for each taxon
% included in the list of microbes analyzed.
...
% is only available in MATLAB R2016b or newer. Older versions of MATLAB will be unable
...
% calculated. The resulting Pareto frontier depicts all possible outcomes
% of co-growth between the two microbes under the given constraints.

% Let us compute the Pareto frontier for five randomly chosen pairs from
% the list of AGORA models.
...
% Perform the Pareto optimality analysis for the five pairs. The shape of
% the computed Pareto frontier, which represents all possible optimal
% solutions of simultaneously optimized growth, depends on the metabolic
% networks of the two joined microbes.
...
% Can you interpret the shapes of the five Pareto frontiers that
% were computed? Are there microbe pairs that are always competing
% with each other? Are there pairs in which one microbe can
% benefit the other at certain points in the curve and vice versa?