...
% 
% Modified for use in modern versions of MatLab by Rui Afonso Tavares, Līva 
% Lukaša & Reinis Muižnieks, 2026.
%% Reviewer(s): 
...
% and each visualisation  layout was saved as a static figure and placed in the 
...
% radius – stands for distance indicator between metabolite of interest and involved 
% reactions (default is 1), (5) direction – stands for algorithm visualization 
% mode ('struc', 'sub', 'prod' or 'both') in order to visualize structure (struc) 
% of reconstructions without FBA data or visualize substrates (sub), products 
% (prod) or substrates and products (both) for models with flux constraints and 
% FBA data (default is 'struc'), (6) excludeMets (optional) – stands for a list 
% of the excludable metabolites as a filter and (7) flux – stands for vector of 
% FBA data of reactions flux distribution (default is vector of x characters for 
% no flux). The last 5 arguments were optional in previous releases, and could 
% be unset but are now required in the updated version. The function draw_by_met 
% ha s 3 outputs: (1) directionRxns – stands for a list of involved reactions 
% depending on the input arguments, (2) involvedMets – stands for a list of involved 
% metabolites depending on the input arguments and (3) deadEnds – stands for alist 
% of dead-end metabolites depending on the input arguments. 
% 
% The layout of the network is generated by using the digraph function built 
% into MatLab. The visual representation of information in the layout (see Fig. 
% 1) allows quick assessment of running processes generally.
% NOTE
% Unlike in previous releases of Paint4Net, which relied on the now depreciated 
% *Biograph* function and variables have been removed from MATLAB since the R2023b 
% version, this version uses *digraph* instead, which while visually different, 
% has similar functionality.
% 
% 
% 
% *Fig. 1* Zoomed fragment of a large model. Reaction nodes (square) contain 
% IDs and flux rates. Metabolite nodes (circle) are marked by IDs. Forward and 
...
% of blocked and exchange reactions are marked by red. Nodes can be chosen by 
% a cursor to see detailed information: (1) Reaction name, number of nodes in 
% and out and (2) ID, name, synonyms and charged formula for a metabolite.  Metabolite 
% etoh_[c]_ is selected by cursor for displaying of detailed information.  
% 
% 
% 
...
% at <https://www.biosystems.lv/paint4net/sample_files https://www.biosystems.lv/paint4net/sample_files>. 
...
% for the command bio_draw_by_rxn.

FBAsolution.x
%% 
% *iii.* execution of the command _[involvedMets, deadEnds] = bio_draw_by_rxn 
% (model, rxns, drawMap, direction, initialMet, excludeMets, flux)_.
% 
% Note: first time running may throw an error, which may be fixed by replacing 
% {''} with "" or {""}, at which point any should work. 
...
load("Paint4Net sample pack/Paint4Net_test_workspace.mat")
%% 
% *ii.* execution of the command _[involvedMets, deadEnds] = bio_draw_by_rxn 
% (model, rxns, drawMap, direction, initialMet, excludeMets, flux)_ which generates 
% a reaction map without flux data. 

[involvedMets, deadEnds] = draw_by_rxn(model, model.rxns, 'true')
%% 
% x
% 
% In this case in the brackets at the square nodes are shown characters (0) 
% indicating that fluxes at steady state were not calculated or applied to the 
% map. This approach is useful in case of reconstruction visualization where fluxes 
% are not calculated. The arrows in the end of the edges point on the forward 
% directions of the reactions in the COBRA model.
%% _*Input arguments of the command bio_draw_by_rxn*_
% The command bio_draw_by_rxn has several input arguments – _model, rxns_, _drawMap_, 
% _direction_, _initialMet_, _excludeMets_, and _flux_. The last 5 arguments are 
% optional; it means that the algorithm of the command bio_draw_by_rxn uses default 
...
[involvedMets, deadEnds] = draw_by_rxn(model, model.rxns, 'true')
%% 
% *iv. optional argument _direction*_
% 
% It is a _string_ type variable that can take value of _'*struc*'_, _'*sub*'_, 
% _'*prod*'_ or _'*both*'_ (default is _'struc'_) indicating a direction for the 
% algorithm of the command bio_draw_by_rxn_._
...
% essential for the command _draw_by_met_  of the Paint4Net v1.4 because the command 
% _draw_by_met_ is calling out the command bio_draw_by_rxn and passing the argument 
% _direction_.
%% 
% *v. optional argument _initialMet*_
% 
...
%% 
% *vi. optional argument _excludeMets*_
% 
...
% 
% >>[Involved_mets, Dead_ends] = draw_by_rxn(model, model.rxns, 'true', 'struc', 
% {''}, {'h[c]', 'h2o[c]', 'atp[c]', 'adp[c]'}, FBAsolution.x)
% 
% 
% 
% 
...
% 
% >>[Involved_mets, Dead_ends] = draw_by_rxn(model, model.rxns, 'true', 'struc', 
% {''}, {'h[c]', 'h2o[c]', 'atp[c]', 'adp[c]', 'nad[c]'}, FBAsolution.x)
%% 
% *vii. optional argument _flux*_
% 
...
FBAsolution.x
%% _*Output of the command bio_draw_by_rxn*_
% The command bio_draw_by_rxn has two output vectors in the result: _involvedMets_ 
% and _deadEnds_.
% 
...
% _*draw_by_met* can be performed but does need flux data, and so will need 
% wither a viable model or a flux space of 0 values equal to the reactions in 
...
% NOTE
% Unlike in previous releases of Paint4Net, which relied on the now depreciated 
% *Biograph* function, in recent releases the _draw_by_met_ function now requires 
% flux data in order to plot metabolite links and reactions within a model.
% 
% The command _draw_by_met_ has several input arguments – _model, metAbbr, drawMap, 
% radius, direction, excludeMets,_ and _flux_. The last 5 were optional in previous 
% releases, but need to be included in recent releases.
% 
% If using previous versions of MatLab that have access to the biograph funtion, 
% it means that the algorithm of the command _draw_by_met_ uses default values 
% of those arguments, so the user can ignore them if additional functionality 
% is not necessary.
% 
% *i. argument _model*_
% 
% This argument stands for a COBRA model in the MATLAB workspace.Load the model 
% and create an FBASolution object, as before, or your model, the model argument 
% represents your model of interest.
% 
% *ii. argument _metAbbr*_
% 
% It is a _cell_ type variable that can take a value that represents the abbreviation 
% of a metabolite in a COBRA model. This argument is the start point for the algorithm 
% of the command _draw_by_met_ for visualization, however, alone creates empty 
% cells as a result

[directionRxns, involvedMets, deadEnds] = draw_by_met(model, {'etoh[c]'})
%% 
% *iii.* For successful execution of the command _[directionRxns, involvedMets, 
% deadEnds] = draw_by_met(model, metAbbr, drawMap, radius, direction, excludeMets, 
% flux)_ the other arguments are necessary.
% 
% These required arguments are:
%% 
% * _*metAbbr -_ metabolite of interest
% * _*drawmap -*_ True or False based on if you want visual output or not
% * *radius -* It is a _double_ type variable that can take a value of natural 
% numbers (1,2,3…n).
% * *direction -* visualize structure (struc) visualize substrates (sub), products 
% (prod) or substrates and products (both)
...
%% 
% *v. argument _direction*_
% 
% It is a _string_ type variable that can take value of _'*struc*'_, _'*sub*'_, 
% _'*prod*'_ or _'*both*'_ (default is _'struc'_) indicating a direction for the 
% algorithm of the command _draw_by_met._ In case of _'struc'_ (structure) the 
% algorithm visualizes all metabolites connected to the specified reactions in 
% the argument _rxns_. The key feature of this function is visualization of all 
% specified reactions not taking in account a steady state fluxes in that way 
% representing the structure of the COBRA model. In case of _'sub'_ (substrates) 
% the algorithm visualizes only those metabolites which are substrates for the 
% specified reactions in the argument _rxns_. This time the algorithm is using 
% a stoichiometric matrix and the steady state fluxes to determine direction of 
% each reaction. The algorithm is using an assumption that only those fluxes which 
% rates are smaller than -10-9 mmol*g-1*h-1or greater than +10-9 mmol*g-1*h-1 
% are non-zero fluxes. In case of _'prod'_ (products) the algorithm visualizes 
% only those metabolites which are products for the specified reactions in the 
% argument _rxns_ but in case of _'both'_ the algorithm visualizes both – substrates 
% and products - for the specified reactions in the argument _rxns_. For both 
% cases the algorithm is using the same rules regarding to calculation of the 
% directions for each reaction as for case of _'sub'_.
% 
% *vi. optional argument _excludeMets*_
% 
% This is the only optional argument in the latest version of Paint4Net, and 
% represents a list of metabolites (default is empty) that will be excluded from 
% the visualization map of the COBRA model in form of the abbreviations of the 
% metabolites separated by a comma. 
% 
% *vii. argument _flux*_
% 
% It is a _double_ type Nx1 size vector of fluxes of reactions where N is number 
% of reactions (default is vector of x). This vector is calculated during the 
% optimization of the objective function and can be accessed through the result 
