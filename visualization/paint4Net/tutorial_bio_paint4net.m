...
% The function bio_draw_by_rxn has 7 input arguments: (1) model – stands for stoichiometric 
...
% The function bio_draw_by_rxn has 2 outputs: (1) involvedMets – stands for a 
% list of involved metabolites depending on the input arguments and (2) deadEnds 
...
%% 
% * _[involvedMets, deadEnds] = bio_draw_by_rxn(model, rxns, drawMap, direction, 
...
%% _Application of command_ bio_draw_by_rxn 
% _*bio_draw_by_rxn*_ can be performed using *option* *A* in case COBRA model 
% optimization results have to be visualized or *option* *B* if the interconnections 
% between metabolites in COBRA file have to be visualized (See Fig. 2).
% 
% 
% 
% *Fig. 2.* The scenarios of an application of the command bio_draw_by_rxn.
%% 
% 
% 
...
[Involved_mets, Dead_ends] = bio_draw_by_rxn(model, model.rxns, 'true', 'struc', {''}, {''}, FBAsolution.x)
...
[involvedMets, deadEnds] = bio_draw_by_rxn(model, model.rxns, 'true')
%% 
% 
% 
...
[involvedMets, deadEnds] = bio_draw_by_rxn(model, rxns)
%% 
...
[involvedMets, deadEnds] = bio_draw_by_rxn(model, model.rxns)
%% 
...
[involvedMets, deadEnds] = bio_draw_by_rxn(model, model.rxns, 'false')
%% 
...
% of the command bio_draw_by_rxn_._
...
[Involved_mets, Dead_ends] = bio_draw_by_rxn(model, model.rxns, 'true', 'struc', {'atp[c]'}, {''}, FBAsolution.x)
...
% because the command _draw_by_met_ is calling out the command bio_draw_by_rxn 
...
[Involved_mets, Dead_ends] = bio_draw_by_rxn(model, model.rxns, 'true', 'struc', {''}, excludeMets)
%% 
...
% used for the function [involvedMets, deadEnds] = bio_draw_by_rxn(model, rxns, 
...
% function [involvedMets, deadEnds] = bio_draw_by_rxn(model, rxns, drawMap, direction, 
...
% the function [involvedMets, deadEnds] = bio_draw_by_rxn(model, rxns, drawMap, 
...
% the hypergraph layout will be generated by Paint4Net using the Bioinformatics 
% Toolbox. The reaction nodes will contain _x_ for flux rates. All interconnecting 
% edges will be in gray because of no FBA data.

[involvedMets, deadEnds] = bio_draw_by_rxn(model, rxns, 'true')
%% 
% 
% 
% *7. Perform FBA.*
...
[Involved_mets, Dead_ends] = bio_draw_by_rxn(model, rxns, 'true', 'struc', {''}, {''}, FBAsolution.x)
%% 
% 
% 
...
[Involved_mets, Dead_ends] = bio_draw_by_rxn(model, model.rxns, 'true', 'struc', {''}, excludeMets, FBAsolution.x)
%% 
% 
% 
